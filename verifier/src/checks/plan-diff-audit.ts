// Session 5 — pre-build plan-diff audit.
//
// Replaces the STUB in atlas/src/lib/workshop-engine.ts
// (requestVerifierMidSessionAudit) with a real check. Called by the Workshop
// approval flow before a plan diff is allowed through the user gate. Surfaces
// breaking changes — deleted phases, reordered phases, removed milestones —
// so the user sees a Verifier verdict alongside the diff preview.
//
// Note on the spec's reference to `atlas_plan_specs`: the live schema doesn't
// have that table — the canonical plan baseline lives either in the latest
// applied row of `plan_diffs` (when at least one diff has been applied) or in
// `.agent/master-plan.md` for the very first run. We accept either as the
// baseline; the caller can pass an override for tests.
 ...
 *      removed milestone.
 *
 * Heuristic #3 is best-effort: it parses the baseline section for lines
 * starting with `- ` or `* ` under `## Milestones` and checks the new body
 * for verbatim presence of each.
 */
...
async function loadBaselinePlanMarkdown(): Promise<string> {
  ...
  // Disk fallback. REPO_ROOT honoured (matches files-exist.ts convention).
...
/**
 * Grabs `## Phase 1.10bb-c` / `### 1.4d` style headers. The phase_id is the
 * trailing dotted token; we tolerate "Phase " prefixes + trailing colons.
 */
...
/** Phase ids in document order. Used to detect reorders. */
...
/**
 * Per-phase milestone bullets. Walks each phase header's body until the next
 * header at the same-or-shallower depth; within that body, finds a
 * `## Milestones` / `### Milestones` block and returns the trimmed bullet
 * text lines.
 */
function extractMilestonesByPhase(markdown: string): Map<string, string[]> {
  ...
  for (const raw of lines) {
    ...
    if (subHeaderMatch) {
      ...
      // A deeper header titled "Milestones" opens the bullet collection.
