...
### Gap 1: o3-judgment
- Severity: `fail`
- Expected: atlas/src/workshop/queue-validator.ts with validateQueueCandidate wired into workshop entrypoint, accompanying unit tests, and documentation update; build integration.
- Actual: No queue-validator implementation, no tests, no modifications to workshop entrypoint; build changes absent.
- Remediation: Create full queue-validator module, integrate gate before mv, add unit tests for all scenarios, ensure build passes.

### Gap 2: gemini-judgment
- Severity: `fail`
- Expected: V3-CODING-INSTRUCTIONS.md updated to document audit-only: true frontmatter flag.
- Actual: V3-CODING-INSTRUCTIONS.md file is missing.
- Remediation: Add or update V3-CODING-INSTRUCTIONS.md to include audit-only flag documentation.
...
Spec `## Acceptance criteria`:
2. `audit-only: true` is documented in `V3-CODING-INSTRUCTIONS.md` as the explicit escape hatch for investigation-style specs.
- Remediation: Create or update the `V3-CODING-INSTRUCTIONS.md` file to include documentation for the `audit-only: true` frontmatter flag. The content for this documentation has already been drafted within the comments of `atlas/src/workshop/queue-validator.ts` and can be moved to the correct file.
