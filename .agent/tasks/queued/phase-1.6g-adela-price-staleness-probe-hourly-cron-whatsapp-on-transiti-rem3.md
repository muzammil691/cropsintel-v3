...
### Gap 1: acceptance-criterion-3
- Severity: `fail`
- Expected: File adela/src/probes/price-staleness.test.ts with 6 unit tests that pass
- Actual: No test file present in shipped code
- Remediation: Create adela/src/probes/price-staleness.test.ts implementing the six specified cases and ensure it passes the existing test runner

### Gap 2: acceptance-criterion-2
- Severity: `fail`
- Expected: Scheduler registers runPriceStalenessProbe with hourly cron (0 * * * *) following existing job pattern
- Actual: File shows import but full cron registration block is not visible; therefore registration is missing or unverified
- Remediation: Add a JobConfig entry (name 'price-staleness-probe', schedule '0 * * * *', fn runPriceStalenessProbe) and wire it through the existing cron/task creation logic
