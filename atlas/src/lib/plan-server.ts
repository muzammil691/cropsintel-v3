...
export async function buildWorkflowGraph(): Promise<WorkflowGraph> {
  ...
  try {
    ...
    console.warn('[plan-server] workflow file unreadable:', err)
    return { nodes: [], edges: [] }
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    else if (line.startsWith('### ')) inOperatingModelsSection = false
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Detect departments mentioned in the body — produce edges
    const referencedDepts = KNOWN_DEPARTMENTS.filter(d =>
      bodyParts.join('\n').toLowerCase().includes(d.toLowerCase().split(/[\s/]/)[0]),
    )
    ...
    // Sequential edge to next workflow
    if (num > 1) {
      edges.push({
        id: `e-flow-${num - 1}-${num}`,
        source: `wf-${num - 1}`,
